...
export class ReportsService {
  ...
  private async generateParcelCSV(parcels: any[], res: Response) {
    ...
    res.setHeader(
      'Content-Disposition',
      'attachment; filename=parcels-report.csv',
    );
    res.send(csvString);
  }
  ...
  private async generateParcelPDF(parcels: any[], res: Response) {
    ...
    res.setHeader(
      'Content-Disposition',
      'attachment; filename=parcels-report.pdf',
    );
  ...
  private async generateAgentCSV(agents: any[], res: Response) {
    ...
    res.setHeader(
      'Content-Disposition',
      'attachment; filename=agents-report.csv',
    );
    res.send(csvString);
  }
  ...
  private async generateAgentPDF(agents: any[], res: Response) {
    ...
    agents.forEach((agent) => {
      ...
      doc.text(
        `${agent.successfulDeliveries}/${agent.totalDeliveries}`,
        320,
        yPosition,
      );
