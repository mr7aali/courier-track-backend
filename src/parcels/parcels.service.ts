...
export class ParcelsService {
  ...
  async updateStatus(id: string, updateStatusDto: UpdateStatusDto, updatedBy: string): Promise<Parcel> {
  ...
  async getParcelsByCustomer(customerId: string, paginationDto: PaginationDto) {
    return this.findAll(paginationDto, { customerId: new Types.ObjectId(customerId) })
  }

  async getParcelsByAgent(agentId: string, paginationDto: PaginationDto) {
    return this.findAll(paginationDto, { agentId: new Types.ObjectId(agentId) })
  }
  ...
  async getParcelStats() {
    ...
    const codParcels = await this.parcelModel.countDocuments({ paymentType: "cod" })
