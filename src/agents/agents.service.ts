...
import type { Agent, AgentDocument } from "./schemas/agent.schema"
...
export class AgentsService {
  ...
  async findAll(paginationDto: PaginationDto, filters?: any) {
    ...
    const [agents, total] = await Promise.all([
      this.agentModel.find(query).populate("userId", "name email phone").sort(sort).skip(skip).limit(limit).exec(),
  ...
  async findOne(id: string): Promise<Agent> {
    const agent = await this.agentModel.findById(id).populate("userId", "name email phone").exec()
  ...
  async updateLocation(id: string, updateLocationDto: UpdateLocationDto): Promise<Agent> {
  ...
  async updateStatus(id: string, status: string): Promise<Agent> {
    const agent = await this.agentModel
      .findByIdAndUpdate(id, { status, lastActiveAt: new Date() }, { new: true })
      .populate("userId", "name email phone")
      .exec()
  ...
  async getAvailableAgents(serviceArea?: string) {
    ...
    return this.agentModel.find(query).populate("userId", "name email phone").sort({ rating: -1 }).exec()
  }

  async updateDeliveryStats(agentId: string, isSuccessful: boolean): Promise<void> {
    const updateQuery = {
      $inc: {
        totalDeliveries: 1,
        ...(isSuccessful ? { successfulDeliveries: 1 } : { failedDeliveries: 1 }),
      },
    }
  ...
  async getAgentStats() {
    ...
    const activeAgents = await this.agentModel.countDocuments({ isActive: true })
    const availableAgents = await this.agentModel.countDocuments({ status: "available" })
