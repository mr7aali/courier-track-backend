...
export class TrackingService {
  ...
  constructor(parcelModel: Model<ParcelDocument>, agentModel: Model<AgentDocument>) {
    this.parcelModel = parcelModel
    this.agentModel = agentModel
  }

  async getParcelLocation(trackingId: string) {
    const parcel = await this.parcelModel.findOne({ trackingId }).populate("agentId", "currentLocation").exec()
    ...
    return {
      ...
      agentLocation: parcel.agentId ? (parcel.agentId as any).currentLocation : null,
      statusHistory: parcel.statusHistory,
      estimatedDeliveryTime: parcel.estimatedDeliveryTime,
    }
  }

  async updateParcelLocation(parcelId: string, latitude: number, longitude: number, address?: string) {
  ...
  async getRouteOptimization(agentId: string) {
    ...
    const optimizedRoute = assignedParcels.map((parcel) => ({
      parcelId: parcel._id,
      trackingId: parcel.trackingId,
      address: parcel.status === "assigned" ? parcel.pickupAddress : parcel.deliveryAddress,
