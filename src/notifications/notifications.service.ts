...
export class NotificationsService {
  ...
  async findAll(paginationDto: PaginationDto, userId?: string) {
    ...
    const [notifications, total] = await Promise.all([
      this.notificationModel.find(query).populate("userId", "name email").sort(sort).skip(skip).limit(limit).exec(),
  ...
  async markAsRead(id: string): Promise<Notification> {
    return this.notificationModel.findByIdAndUpdate(id, { isRead: true, readAt: new Date() }, { new: true })
  }
  ...
  async sendParcelStatusNotification(userId: string, trackingId: string, status: string): Promise<void> {
  ...
  async sendDeliveryNotification(userId: string, trackingId: string): Promise<void> {
